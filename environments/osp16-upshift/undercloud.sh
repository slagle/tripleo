...
sudo dnf install -y python3-tripleoclient git tmux expect bash-completion ceph-ansible
...
openstack complete | grep -v INFO | grep -v DEBUG | sudo tee /etc/bash_completion.d/openstack
